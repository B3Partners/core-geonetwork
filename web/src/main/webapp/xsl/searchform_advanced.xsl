		...
		<span title="{/root/gui/strings/searchhelp/keywords}">
			<xsl:choose>
				<xsl:when test="not($remote)">
				  <input id="themekey" name="themekey" onClick="popSelector(this,'keywordSelectorFrame','keywordSelector','portal.search.keywords?mode=selector&amp;sortByKeyword=true&amp;keyword','themekey');" class="content" size="31" value="{/root/gui/searchDefaults/themekey}"/>
				</xsl:when>
				<xsl:otherwise>
