...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd">
  ...
  <xsl:template match="/">
	...
	<!-- If document starts with ISO19139, don't do an ESRI -> ISO19139 conversion. 
         If a child element of the root element is MD_Metadata ISO19139, only copy that element.

		 Use this stylesheet with the local filesystem harvester (configure this XSLT to apply
         to harvested records) to harvest XML files containing either:
         - ISO 19139 gmd:MD_Metadata root
         - ISO 19139 gmd:MD_Metadata in any location in the document (like wrapped as a child of <metadata>)
         - ESRI metadata which will be converted to ISO19115 (this may not work correctly, YMMV)
	-->
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
		<!-- Produce ISO19115 output -->
		<xsl:call-template name="EsriToISO19115"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <xsl:template name="EsriToISO19115">
    <Metadata>
      ...
      <xsl:choose>
        <xsl:when test="metadata/mdContact">
          <xsl:apply-templates select="metadata/mdContact"/>
        </xsl:when>
        <xsl:otherwise>
		  <!--xsl:for-each select="metadata/idinfo/ptcontac">
			<xsl:apply-templates select="metadata/idinfo/ptcontac" mode="mdContact"/>
          </xsl:for-each-->
        </xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:template match="DQRelIntPosAcct | DQQuanAttAcc | DQDomConsis | DQGridDataPosAcc | DQTempValid | DQAbsExtPosAcc | DQConcConsis | DQCompComm | DQFormConsis | DQTopConsis | DQAccTimeMeas | DQNonQuanAttAcc | DQThemClassCor | DQCompOm | DQTempConsis">
    <xsl:variable name="name" select="local-name(.)"/>
    <xsl:element name="{$name}">
  ...
  <xsl:template match="vertUoM | axisUnits | falENUnits | UomArea | UomTime | UomLength | UomVolume | UomVelocity | UomAngle | UomScale | valUnit">
    <xsl:variable name="name" select="local-name(.)"/>
    <xsl:element name="{$name}">
  ...
  <xsl:template match="portCatCit | asName | identAuth | thesaName | idCitation | srcCitatn | evaluationProcedure | conSpec | paraCit | catCitation">
  ...
  <xsl:template match="distorTran | distTranOps">
    ...
    <xsl:element name="distTranOps">
      ...
      <!-- because of an error in ArcCatalog, we will match on offLineMed -->
    </xsl:element>
  </xsl:template>
  <!-- Template with identAuth identCode -->
  <xsl:template match="refSysID | projection | ellipsoid | datum | datumID | geoId | RS_Identifier | MdIdent">
    <xsl:variable name="name" select="local-name(.)"/>
    <xsl:element name="{$name}">
  ...
  <!-- Updated xslt -->
  <xsl:template match="ptcontac" mode="mdContact">
    <xsl:element name="mdContact">
      <rpIndName>
  ...
  <xsl:template match="ptcontac" mode="idPoC">
    <xsl:element name="idPoC">
      <rpIndName>
        <xsl:value-of select="cntinfo/cntperp/cntper"/>
      </rpIndName>
      <rpOrgName>
  ...
  <xsl:template name="cntper">
    <rpIndName>
      <xsl:value-of select="."/>
    </rpIndName>
  </xsl:template>

  <xsl:template name="cntorg">
    <rpOrgName>
      <xsl:value-of select="."/>
    </rpOrgName>
  </xsl:template>

  <xsl:template name="cntpos">
    <rpPosName>
      <xsl:value-of select="."/>
    </rpPosName>
  </xsl:template>
  ...
  <xsl:template match="idinfo">
    <dataIdInfo>
      ...
      <status>
        <ProgCd>
          <xsl:attribute name="value">
            <xsl:value-of select="status/progress"/>
          </xsl:attribute>
        </ProgCd>
      </status>
      <xsl:apply-templates select="ptcontac" mode="idPoC"/>
      ...
      <xsl:choose>
        <xsl:when test="/metadata/distinfo/stdorder/digform/digtinfo">
          <xsl:for-each select="/metadata/distinfo/stdorder/digform/digtinfo">
            <dsFormat>
              <formatName>
        ...
        <xsl:otherwise>
          <xsl:for-each select="/metadata/dataIdInfo/dsFormat">
            <xsl:apply-templates select="."/>
          </xsl:for-each>
        </xsl:otherwise>
      </xsl:choose>

      <xsl:for-each select="/metadata/dataIdInfo/idSpecUse">
        <idSpecUse>
      ...
      <xsl:for-each select="/metadata/dataIdInfo/resMaint">
        <xsl:apply-templates select="."/>
      </xsl:for-each>
      ...
      <graphOver>
        <bgFileName>small.bmp</bgFileName>
        <bgFileDesc>thumbnail</bgFileDesc>
        <bgFileType>small</bgFileType>
      </graphOver>

      <xsl:for-each select="/metadata/dataIdInfo/spatRpType">
        <xsl:apply-templates select="."/>
      </xsl:for-each>

      <xsl:for-each select="/metadata/dataIdInfo/dataScale">
        <dataScale>
      ...
      <xsl:choose>
        <xsl:when test ="descript/langdata">
          <dataLang>
            <languageCode>
              <xsl:attribute name="value">
                <xsl:value-of select="descript/langdata"/>
              </xsl:attribute>
            </languageCode >
          </dataLang>
        </xsl:when>
      ...
      <xsl:apply-templates select="/metadata/dataIdInfo/dataChar"/>
      ...
      <xsl:for-each select="/metadata/dataIdInfo/geoDesc">
        <geoDesc>
          <xsl:apply-templates select="exTypeCode"/>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:apply-templates select="/metadata/dataIdInfo/envirDesc"/>
        </xsl:otherwise>
      </xsl:choose>
      ...
      <xsl:choose>
        <xsl:when test ="descript/supplinf">
          <suppInfo>
            <xsl:value-of select="descript/supplinf"/>
          </suppInfo>
        </xsl:when>
        <xsl:otherwise>
          <xsl:apply-templates select="/metadata/dataIdInfo/suppInfo"/>
        </xsl:otherwise>
      </xsl:choose>
  </dataIdInfo>
  </xsl:template>
  ...
  <!-- When no pubdate is found in the source document, the whole citation object is neglected/skipped!!! -->
  <xsl:template match="citation">
    <xsl:if test="citeinfo/pubdate">
      <xsl:element name="idCitation">
        ...
        <resRefDate>
          <refDate>
            <xsl:value-of select="citeinfo/pubdate"/>
          </refDate>
          <refDateType>
            <DateTypCd value="001"/>
          </refDateType>
        ...
        <PresFormCd>
          <xsl:attribute name="value">
            <xsl:value-of select="citeinfo/geoform"/>
          </xsl:attribute>
        </PresFormCd>
        <datasetSeries>
          <seriesName>
            <xsl:value-of select="citeinfo/serinfo/sername"/>
          </seriesName>
          <issId>
            <xsl:value-of select="citeinfo/serinfo/issue"/>
          </issId>
        </datasetSeries>
        <otherCitDet>
          <xsl:value-of select="citeinfo/othercit"/>
        </otherCitDet>
      </xsl:element>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="cntinfo">
    <rpCntInfo>
      <cntPhone>
        <voiceNum>
          <xsl:value-of select="cntvoice" />
        </voiceNum >
        <faxNum>
          <xsl:value-of select="cntfax" />
        </faxNum >
      </cntPhone>
      <cntAddress>
        ...
        <xsl:apply-templates select="cntaddr/city"/>
        <adminArea>
        ...
        <xsl:apply-templates select="cntaddr/country"/>
